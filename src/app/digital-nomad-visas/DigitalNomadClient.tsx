...
import { Search, Globe, TrendingUp, ArrowUpDown, MapPin, Shield, Wind, Landmark, CheckCircle, Smile, ExternalLink, ArrowRight, Plane } from 'lucide-react';
...
export default function DigitalNomadClient({ nomadCountries }: DigitalNomadClientProps) {
  ...
  const processedCountries = [...nomadCountries]
    .filter(country => 
  ...
  return (
    <div className="space-y-12 pb-20">
      ...
      <section className="text-center py-16 px-4 relative">
        ...
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 font-semibold mb-6 border border-blue-200 dark:border-blue-800/50 shadow-sm">
          <Plane className="w-5 h-5" /> 2026 Updated Guide
        </div>
        <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight mb-6 text-glow">
          Digital Nomad <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-emerald-400">Visas</span>
        </h1>
        <p className="text-lg md:text-xl text-slate-700 dark:text-slate-300 max-w-2xl mx-auto mb-10 leading-relaxed">
          Compare {nomadCountries.length} countries offering remote work visas and permits. Analyze their tax burdens, safety, and economic freedom to find your perfect base.
        </p>
        ...
        <div className="max-w-2xl mx-auto relative group mb-12">
          ...
          <input 
            ...
            className="block w-full pl-14 pr-4 py-5 glass-panel rounded-2xl text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all text-xl shadow-2xl border border-black/10 dark:border-white/10 bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl" 
            placeholder="Search for a country offering a nomad visa..."
          />
        </div>

        <div className="glass-panel rounded-2xl overflow-hidden border border-black/10 dark:border-white/10 shadow-2xl max-w-6xl mx-auto bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl">
          ...
          {/* MOBILE CARDS */}
          <div className="md:hidden divide-y divide-black/10 dark:divide-white/10 text-left">
            {processedCountries.map((country) => {
              const taxes = country.indicators?.['wb_taxes']?.value;
              const freedom = country.indicators?.['heritage_economic_freedom']?.value;
              const happiness = country.indicators?.['whr_happiness_index']?.value;
              ...
              return (
                <div key={country.id} className="p-5 flex flex-col gap-4 hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                  ...
                  <div className="flex flex-col gap-2 mt-2">
                    <Link 
                      href={`/country/${country.slug}`}
                      className="w-full flex items-center justify-center px-4 py-2.5 text-sm font-bold text-white bg-blue-600 hover:bg-blue-500 rounded-xl transition-all shadow-md shadow-blue-500/20"
                    >
                      Analyze Destination
                    </Link>
